...
4. **후보별 응답 저장**
   ...
   - Room 조회는 현재 Bearer 토큰의 참여자 응답만 `myResponses`로 반환하여 새로고침·재입장 후 Client가 저장 상태를 복원하게 한다. 빠른 테스트 입력은 기존 후보별 응답 PUT을 사용하며, 입력 적용과 실제 저장을 구분한다.
...
- 새로고침·재입장 후에도 현재 참여자의 활성 후보 응답을 복원하고, 미응답을 기본 응답으로 채우지 않는다.
